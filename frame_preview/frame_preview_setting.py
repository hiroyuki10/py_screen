    ...
    def event_save(self, event):
        ...
        if os.path.exists(save_path):
            ...
            if messagebox.askyesno(
                "警告", "フォルダを上書きしていいですか？", icon="warning"
            ):
                print("yes")
                self.delete_files()
                ...
                t1 = threading.Thread(
                    name="rename worker1",
                    target=lambda param=_param: self.rename_files(save_path, param),
                )
                t1.start()
                return
            else:
        ...
        t1 = threading.Thread(
            name="rename worker1",
            target=lambda images=_images: self._save_images(save_path, images),
        )
        t1.start()
