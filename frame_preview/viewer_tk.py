...
class ViewerTk(tkinter.Toplevel):
    ...
    def _update(self):
        self.title(str(self.image_select_number+1)+"/"+str(len(Param.get_source_images())))

        num = self.image_select_number
        if num < len(Param.get_source_images()) - 1:
            self.selected_image = self._resize_image(Param.get_source_image(self.image_select_number), self.w, self.h)
            self.selected_image2 = self._resize_image(Param.get_source_image(self.image_select_number+1), self.w, self.h)
            self.canvas.create_image(self.w/4*3, self.h/2, image=self.selected_image)  # 画像の中心座標を設定する
            self.canvas.create_image(self.w/4*1, self.h/2, image=self.selected_image2)  # 画像の中心座標を設定する
        else:
            self.selected_image = self._resize_image(Param.get_source_image(self.image_select_number), self.w, self.h)
