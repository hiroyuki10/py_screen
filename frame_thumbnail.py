...
class FrameThumbnail(FrameScroll):
    ...
    def update(self, pil_images, _names=None):
        ...
        if self.updating_flag:
            if self.thread_kill_flag:
                ...
                t2 = threading.Thread(
                    target=lambda images=pil_images, names=_names: self._assert_thread_kill_flag(
                        images, names
                    )
                )
                t2.start()
            ...
            t1 = threading.Thread(
                target=lambda images=pil_images, names=_names: self.thread_update(
                    images, names
                )
            )
            t1.start()
    ...
    def gen_button(self, image, name=None):
        ...
        if _row == 0:
            _row = 1
        th_image = self._resize_image(
            image, int(self.button_size[0]), int(self.button_size[1])
        )
        ...
        button.grid(
            row=int((len(self.buttons)) / _row), column=int((len(self.buttons)) % _row)
        )
    ...
    def _bind_button(self, button, _name):
        button.bind(
            "<Button-1>",
            lambda event, num=len(self.buttons), name=_name: self._callback(
                event, num, name
            ),
        )
