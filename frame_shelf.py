...
class ShelfListScroll(FrameThumbnail):
    ...
    def _bind_button(self, button, _name):
        ...
        button.bind(
            "<Button-3>",
            lambda event, num=len(self.buttons), name=_name: self._callback_2(
                event, num, name
            ),
        )
...
class FrameShelf(tkinter.LabelFrame):
    ...
    def create_controller(self, _master):
        ...
        self.chkbtn_only_image = tkinter.Checkbutton(
            master=frame, text="only image", variable=self.flag_only_image
        )
        ...
        self.chkbtn_show_thumbnail = tkinter.Checkbutton(
            master=frame, text="show thumbnail", variable=self.flag_show_thumbnail
        )
